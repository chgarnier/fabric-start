...
class OrganizationManager {
    ...
    constructor(name, domainName, legacyId, rootDirectory, isOrderer, isMain, peersOptions) {
        ...
        for (let [index, peerOptions] of peersOptions.entries()) {
            let peerName = `peer${index}.${this.name}.${this.domainName}`;  //Necessary to match certificates generated by cryptogen
            if (peerOptions.isOrderer) {
                this.peers.push(new OrdererPeerManager(peerName, this.name, this.domainName, peerOptions.isMain, peerOptions.ec2Type));
            }
            else {
